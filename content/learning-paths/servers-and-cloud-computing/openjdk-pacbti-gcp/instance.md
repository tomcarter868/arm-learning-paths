...
![Screenshot of the Google Cloud Console VM creation page. The C4A series and c4a-standard-4 machine type are selected, confirming creation of an Arm-based C4A instance.#center](images/gcp-vm.png "Creating a Google Axion C4A VM in the Google Cloud Console")
...
![Screenshot of the Google Cloud Console VM instances list. The SSH button is highlighted for a running C4A instance, showing how to open a terminal session on the new VM.#center](images/gcp-ssh.png "Connecting to a running C4A VM using SSH")

A new browser window opens with a terminal connected to your VM.

![Screenshot of a browser-based terminal window with a command prompt on a SUSE Linux VM running on Google Axion C4A. This terminal is used to install OpenJDK and run the PAC/BTI validation script.#center](images/gcp-shell.png "Terminal session connected to the VM")
